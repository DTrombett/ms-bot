...
export class Clash extends Command {
	...
	// 							type: ComponentType.Button,
	// 							emoji: { name: "⬅️" },
	// 							custom_id: `clash-members-${id}-${clan.tag}-${order}-${page - 1}`,
	// 							disabled: !page,
	// 							style: ButtonStyle.Primary,
	// 						},
	// 						{
...
Ricevute: **${player.donationsReceived.toLocaleString()}**
	...
	// 			RETURNING brawlNotifications, clashTag`,
	// 	)
	// 		.bind(id, NotificationType[type])
	// 		.first<{ brawlNotifications: number; clashTag: string | null }>();
	...
	// ) => {
	// 	const result = await env.DB.prepare(
	// 		"SELECT brawlNotifications, clashTag FROM Users WHERE id = ?",
	// 	)
	// 		.bind(id)
	// 		.first<{ brawlNotifications: number; clashTag: string | null }>();
